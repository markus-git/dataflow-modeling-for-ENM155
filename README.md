...
An example of the entire development process, that is, writing a configuration file, generating a graph from that file and outputting an input/output solution, can be found in the class `Main`. Be sure to read through the example to get overview of how the library works.
...
Graphs are specified by the interfaces `Graph`, `Vertex`, and `Edge` found under `enm155/src/graph`. An implementation of these can be found under `enm155/src/graph/directed`. Make sure to read through these and understand them. An implementation of the input/output algorithm can be then found under `calculateDemands` in `MyGraph`.
